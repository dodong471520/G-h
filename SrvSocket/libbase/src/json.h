...
#if		!defined(JSON_MAX_STRING_LENGTH)
#	define JSON_MAX_STRING_LENGTH 0x10000
#endif
...
/**
Outputs a new UTF8 c-string which replaces all escaped characters with their real versions
@param text an UTF8 char text string
@return an UTF-8 c-string holding the same text string but with unescaped characters
**/
...
@return a pointer to the first label holding a text equal to text_label or NULL if there is no such label or if object has no children
**/
	json_t *json_find_first_label_ignore_case (const json_t * object, const char *text_label);

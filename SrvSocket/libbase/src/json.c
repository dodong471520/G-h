...
{
	json_t *new_object;
	//size_t length;		// hezhihuang: unused, 2008.11.20.
	...
	/* 	hezhihuang: escape string first, 2008.11.20 */
	new_object->text = json_escape((char*)text);
	/*
...
/* hezhihuang: unescape string, 2008.11.20 */
char *
json_unescape (const char * text)
{
	...
	for (i = 0; i < length; i++)
	{
		if (text[i] == '\\')
		{
			...
			switch(text[i])
			{
			case '\"':			rcs_catc (output, '\"');break;
			case '\\':			rcs_catc (output, '\\');break;
			case '/':			rcs_catc (output, '/');break;
			case 'b':			rcs_catc (output, '\b');break;
			case 'f':			rcs_catc (output, '\f');break;
			case 'n':			rcs_catc (output, '\n');break;
			case 'r':			rcs_catc (output, '\r');break;
			case 't':			rcs_catc (output, '\t');break;
			default:
				{
					rcs_catc (output, '\\');
					rcs_catc (output, text[i]);
...
int
lexer (const char *buffer, char **p, unsigned int *state, rcstring ** text)
{
	assert (buffer != NULL);
	assert (p != NULL);
	assert (state != NULL);
	...
	while (**p != '\0')
	{
		...
		case 0:	/* Root document */
			{
				...
				case '9':
					...
					*state = 19;	/* number: decimal followup */
					...
					/* + hezhihuang: comments support, 2008.11.20 */
				case '/':
					*state = 60;	/* comment begin. */
					break;
					/* - hezhihuang: comments support ended, 2008.11.20 */
		...
		case 63:	/* comment: c comment close */
			{
				if('/' == **p)
				{
					*state	= 0;
...
/*                      jsps->state = 0;        // redundant*/
		...
		state60:
	{
		...
		{
		case '/':
			jsps->state = 61;	/* cpp comment: */
			break;
		case '*':
			jsps->state = 62;	/* c comment: */
			break;
		default:
			return JSON_ILLEGAL_CHARACTER;
			break;
		...
		state63:
	{
		if ('/' == c)
		{
			jsps->state = 0;	/* c comment ended */
		}
		else
		{
			jsps->state = 62;	/* c comment content */
		}
		return JSON_OK;
	}
	/* - hezhihuang: comments support ended, 2008.11.20 */
...
json_find_first_label (const json_t * object, const char *text_label)
{
	...
	if(NULL == object || NULL == text_label)
	{
		return NULL;
	}
	...
	cursor = object->child;
	text_len = strlen(text_label);
	while (cursor != NULL)
	{
		if(strlen(cursor->text) == text_len)
		{
			if (strncmp (cursor->text, text_label, text_len) == 0)
				return cursor;
		}
		cursor = cursor->next;
	}
...
json_t *
json_find_first_label_ignore_case (const json_t * object, const char *text_label)
{
	json_t *cursor;
	size_t text_len;
	...
	while (cursor != NULL)
	{
		if(strlen(cursor->text) == text_len)
		{
			if (strncasecmp(cursor->text, text_label, strlen (text_label)) == 0)
				return cursor;
		}
		cursor = cursor->next;
	}

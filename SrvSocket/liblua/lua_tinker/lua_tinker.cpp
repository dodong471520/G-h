...
//#include "stdafx.h"
#include "..\export\lua_tinker.h"

#include <Windows.h>

/*
#if defined(_DEBUG) || defined(DEBUG)
#pragma comment(lib,"Lua5_d.lib")
#else
...
{
	char temp[64];
	sprintf_s(temp, sizeof(temp),"%I64d", *(__int64*)lua_topointer(L, 1));
...
{
	char temp[64];
	sprintf_s(temp, sizeof(temp), "%I64u", *(unsigned __int64*)lua_topointer(L, 1));
	lua_pushstring(L, temp);
	return 1;
}
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	{
		//printf("%s\n", text);
        MessageBoxA(NULL,text,"ScriptErr",MB_OK);
		lua_pop(L, 1);
	}
}

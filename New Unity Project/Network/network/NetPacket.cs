...
namespace Network
{
    class NetPacket
    {
        ...
        public List<byte> GetBuffer()
        {
            ...
            ret.Add(m_pktFlag);
            ret.AddRange(BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)m_pktSeq)));
            ret.AddRange(BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)m_pktDataSize)));
        ...
        public void Encrypt(byte[] seed, int len)
        {
            /* encrypt here */
            int i, k;
            for (i = 0, k = 0; i < m_pktDataSize; i++, k++)
            {
                if (k == len) k = 0;
                m_buffer[i] ^= seed[k];
            }
        ...
        public int BeginRead(List<byte> readByte)
        {
            ...
            m_pktSeq =(ushort)IPAddress.NetworkToHostOrder((short)(BitConverter.ToUInt16(readByte.ToArray(), 1)));
            m_pktDataSize = (ushort)IPAddress.NetworkToHostOrder((short)BitConverter.ToUInt16(readByte.ToArray(), 3));

            if (m_pktFlag >> 4 != PROTOCOL_FLAG)
            {
                TcpSession.Sys_Log("NetPacket Flag Error");
                return -1;
            }
            if (m_pktDataSize <= 0 || m_pktDataSize > CmdPacket.MAX_CMD_PACKET_SIZE)
            {
                TcpSession.Sys_Log("NetPacket Size Error : s = " + m_pktDataSize);
                return -1;
            }

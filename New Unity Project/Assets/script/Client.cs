...
public class Client : MonoBehaviour {
    ...
    void OnGUI()
    {
        ...
        sendString = GUILayout.TextArea(sendString);
        if (GUILayout.Button("Send"))
            ActionSay(sendString);
    }
    int m_connTime=0;
    void ActionSay(string word)
    {
        m_sendCmdPkt.BeginWrite();
        m_sendCmdPkt.WriteShort(22);
        m_sendCmdPkt.WriteString(word);
        SendPacket(m_sendCmdPkt, true, false);
    }
    bool Connect(string remote, int port)
    {
        Security.PrefetchSocketPolicy(remote, port);
    ...
    void KeepAlive(int now)
    {
        m_sendCmdPkt.BeginWrite();
        m_sendCmdPkt.WriteShort(SYS_PACKET_KEEP_ALIVE);
        m_sendCmdPkt.WriteInt(now);
        SendPacket(m_sendCmdPkt,true, true);
    }
    ushort m_seq=0;
    ...
    void FlushData()
    {
        ...
        if (m_writeBuf.Count == 0)
         ...
         {
             m_lastSendTime = System.Environment.TickCount;
